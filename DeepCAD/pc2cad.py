...
from utils import cycle, ensure_dir, ensure_dirs, read_ply, write_ply
...
if not args.test:
    ...
    save_dir = os.path.join(
        cfg.exp_dir, "results/fake_z_ckpt{}_num{}_pc".format(args.ckpt, args.n_samples)
    )
    ...
    for i, data in enumerate(pbar):
        ...
        for j in range(pred_z.shape[0]):
            save_folder_ply = os.path.join(save_dir, data["id"][j].split("/")[0])
            print(save_folder_ply)
            if not os.path.exists(save_folder_ply):
                os.makedirs(save_folder_ply)
    ...
    save_path = os.path.join(
        cfg.exp_dir, "results/fake_z_ckpt{}_num{}.h5".format(args.ckpt, args.n_samples)
    )
