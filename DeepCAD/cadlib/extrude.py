...
class Extrude(object):
    ...
    def numericalize(self, n=256):
        ...
        self.extent_one = (
            ((self.extent_one + 1.0) / 2 * n)
            .round()
            .clip(min=0, max=n - 1)
            .astype(np.int)
        )
        self.extent_two = (
            ((self.extent_two + 1.0) / 2 * n)
            .round()
            .clip(min=0, max=n - 1)
            .astype(np.int)
        )
        ...
        self.sketch_pos = (
            ((self.sketch_pos + 1.0) / 2 * n)
            .round()
            .clip(min=0, max=n - 1)
            .astype(np.int)
        )
        self.sketch_size = (
            (self.sketch_size / 2 * n).round().clip(min=0, max=n - 1).astype(np.int)
        )
...
class CADSequence(object):
    ...
    def random_transform(self):
        for item in self.seq:
            ...
            translate = (
                np.array([random.randint(-5, 5), random.randint(-5, 5)], dtype=np.int)
                + 128
            )
